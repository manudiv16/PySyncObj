...
parser.add_argument(
    "revA",
    help=(
        'path or git revision for the "old" version. When it is a path, it must be the'
        " directory containing the pysyncobj package. When it is a git revision, the"
        " parent directory of the directory containing this script must be the git"
        " repository, and this repository must contain the revision (i.e. run this"
...
parser.add_argument(
    ...
    default=None,
    help=(
        "Seed for PRNG. Using the same seed value produces the exact same order of"
        " operations *in this test script*, i.e. outside of PySyncObj. Everything"
        " inside the cluster, e.g. which node is elected leader and when, is"
        " essentially still completely random."
    ),
)
...
if args.cycles < args.processes * 10:
    print(
        "Needs at least ten times as many cycles as there are processes to get useful"
        " results",
        file=sys.stderr,
    )
    sys.exit(1)
...
for i in range(args.cycles):
    cmd = random.choice(
        ("increment", "increment", "increment", "increment", "compare")
    )  # 80 % increment, 20 % compare
    ...
    with cd(tmpdirname):
        ...
        for revArg, revTarget in ((args.revA, "revA"), (args.revB, "revB")):
            if os.path.isdir(os.path.join(workingDir, revArg)):
                ...
                if (
                    subprocess.call(
                        [
                            "rsync",
                            "-a",
                            os.path.join(workingDir, revArg, ""),
                            os.path.join(revTarget, ""),
                        ]
                    )
                    != 0
                ):
                ...
                with cd(
                ...
                ):  # TODO: Replace with GIT_DIR environment variable or something
                    gitProc = subprocess.Popen(
                        ["git", "archive", revArg], stdout=subprocess.PIPE
                    )
                    tarProc = subprocess.Popen(
                        [
                            ...
                            "-x",
                            "-C",
                            os.path.join(tmpdirname, revTarget),
                            "--strip-components",
                            "1",
                            "pysyncobj",
                        ],
                        stdin=gitProc.stdout,
                    )
                    gitProc.stdout.close()
                    tarProc.communicate()
                    if tarProc.returncode != 0:
                        print(
                            "git or tar of {} failed".format(revTarget), file=sys.stderr
                        )
                        sys.exit(1)
            ...
            if (
                subprocess.call(
                    [
                        ...
                        "-a",
                        os.path.join("revA", ""),
                        os.path.join("proc{}".format(i), "pysyncobj", ""),
                    ]
                )
                != 0
            ):
            ...
            if (
                subprocess.call(
                    [
                        "rsync",
                        "-a",
                        os.path.join(workingDir, "proc.py"),
                        os.path.join("proc{}".format(i), ""),
                    ]
                )
                != 0
            ):
        ...
        try:
            ...
            for i in range(args.processes):
                with cd("proc{}".format(i)):
                    procs.append(
                        subprocess.Popen(
                            ["python3", "proc.py", addrs[i]]
                            + [addrs[j] for j in range(args.processes) if j != i],
            ...
            restart = (
                -1
            )  # Variable for when to restart a process; set to 3 on the 'upgrade' command, counted down on each command, the upgraded process is restarted when it reaches zero
            upgradingProcId = None  # The procId that is currently upgrading
            for procId, command in commands:
                if command == "increment":
                    assert procId != upgradingProcId, "previous upgrade hasnt finished"
                    ...
                    if not ok:
                        print(
                            "Didnt find at least {} matching logs".format(
                                args.processes // 2 + 1
                            ),
                            file=sys.stderr,
                        )
                        for i in range(args.processes):
                            print(
                                "proc{} log: {}".format(i, logs[i].decode("utf-8")),
                                file=sys.stderr,
                            )
                        sys.exit(1)
                elif command == "upgrade":
                    assert upgradingProcId is None, "previous upgrade hasnt finished"
                    ...
                    if (
                        subprocess.call(
                            [
                                ...
                                "-a",
                                os.path.join("revB", ""),
                                os.path.join("proc{}".format(procId), "pysyncobj", ""),
                            ]
                        )
                        != 0
                    ):
                        print(
                            "rsync of revB to proc{} failed".format(procId),
                            file=sys.stderr,
                        )
                        sys.exit(1)
                ...
                if restart == 0:
                    ...
                    with cd("proc{}".format(upgradingProcId)):
                        procs[upgradingProcId] = subprocess.Popen(
                            ["python3", "proc.py", addrs[upgradingProcId]]
                            + [
                                addrs[j]
                                for j in range(args.processes)
                                if j != upgradingProcId
                            ],
                            stdin=subprocess.PIPE,
                            stdout=subprocess.PIPE,
                            bufsize=0,
            ...
            for i in range(args.processes):
                ...
                procs[i].stdin.flush()
            counters = [
                int(procs[i].stdout.readline().strip()) for i in range(args.processes)
            ]
            ...
            if not all(x == logs[0] for x in logs):
                ...
                print(
                    "ERROR: not all counters are equal to the expected value {}: {}".format(
                        counter, counters
                    ),
                    file=sys.stderr,
                )
