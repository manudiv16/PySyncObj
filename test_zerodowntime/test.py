...
parser.add_argument(
    ...
    type=int,
    default=None,
    help="Seed for PRNG. Using the same seed value produces the exact same order of operations *in this test script*, i.e. outside of PySyncObj. Everything inside the cluster, e.g. which node is elected leader and when, is essentially still completely random.",
)
...
if args.cycles < args.processes * 10:
    print(
        "Needs at least ten times as many cycles as there are processes to get useful results",
        file=sys.stderr,
    )
    sys.exit(1)
        ...
        try:
            ...
            for procId, command in commands:
                if command == "increment":
                    assert procId != upgradingProcId, (
                        "previous upgrade hasn" "t finished"
                    )
                    ...
                    if not ok:
                        print(
                            "Didn"
                            "t find at least {} matching logs".format(
                                args.processes // 2 + 1
                            ),
                            file=sys.stderr,
                        )
