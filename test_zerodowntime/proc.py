...
def main(argv=sys.argv[1:]):  # , stdin = sys.stdin):
    ...
    print(
        "{} ready at {}".format(selfAddr, pysyncobj.testrevision.rev), file=sys.stderr
    )
        ...
        if line == "wait":
            ...
            while True:
                try:
                    ...
                    print(
                        "{} increment yielded SyncObjException with error code {},"
                        " retrying".format(selfAddr, e.errorCode),
                        file=sys.stderr,
                    )
                else:
                    break
